...
export default function Product({ product }) {
    ...
    return (
        <div className={`flex flex-row h-20 items-center ${isDeleted ? "hidden" : "block"}  border-b-2 border-gray-200 pb-4 mb-4`}>
            ...
            <div className="overflow-hidden truncate">{product.productName}</div>

            <span className="min-w-32 text-center">{product.price.toLocaleString('vi', { style: 'currency', currency: 'VND' })}</span>
            <span className="min-w-32 text-center">{product.stock}</span>
            <span className="min-w-32 text-center ">{product.order}</span>
            <span className="min-w-32 text-center">{new Date(product.publishedOn).toLocaleString('vi', { day: 'numeric', month: 'numeric', year: 'numeric' })}</span>
