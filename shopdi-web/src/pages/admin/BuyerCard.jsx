...
export default function userCard({ status }) {
    ...
    function onBlock(status,userId) {
        status === 'ACTIVE' ?
        PUT(`users/${userId}/ban`).then(res => {
            ...
            :
        PUT(`users/${userId}/unban`).then(res => {
            if (res.code === 'OK') {
                console.log(res)
                GET('users').then(res => {
                    if (res.code === 'OK') {
                        ...
                        {showMenu && idCard === user.username && (
                            ...
                            >
                                <ul>
                                    ...
                                    <li
                                        onClick={() => onBlock(user.status,user.userId)}
                                    ...
                                    >
                                        ...
                                        <p className="pl-2 text-[#D0342C]">{user.status === "ACTIVE" ? 'Blocked' : 'Unblock'}</p>
                                    </li>
                                    <li
