...
public interface CartItemRepository extends JpaRepository<CartItem, Long>, JpaSpecificationExecutor<CartItem> {
    @Query("SELECT ci FROM CartItem ci WHERE ci.cart=:cart AND ci.product=:product AND ci.variant=:variant AND ci.userId=:userId")
    CartItem isCartItemExist(@Param("cart") Cart cart, @Param("product") Product product, @Param("variant") String variant, @Param("userId") Long userId);

    CartItem findByCartAndProductAndVariant(Cart cart, Product product, String variant);
